...
using MyFirstProjectBoilerPlate.Users;
using Castle.MicroKernel.Registration;
using Effort;
using EntityFramework.DynamicFilters;

namespace MyFirstProjectBoilerPlate.Tests
{
    public abstract class MyFirstProjectBoilerPlateTestBase : AbpIntegratedTestBase<MyFirstProjectBoilerPlateTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MyFirstProjectBoilerPlateTestBase()
        {
        ...
        protected void UsingDbContext(Action<MyFirstProjectBoilerPlateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyFirstProjectBoilerPlateDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyFirstProjectBoilerPlateDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyFirstProjectBoilerPlateDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyFirstProjectBoilerPlateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyFirstProjectBoilerPlateDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyFirstProjectBoilerPlateDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyFirstProjectBoilerPlateDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyFirstProjectBoilerPlateDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyFirstProjectBoilerPlateDbContext, Task<T>> func)
        {
            T result;
